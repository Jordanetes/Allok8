...
const cleaner = {
    ...
    podMetrics(pod, usedCPU, usedMemory) {
    ...
    podClean(req, res, next) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            for (let j = 0; j < pods.length; j++) {
                const podData = this.podMetrics(pods[j], nodeData[nodeName].usedCPU, nodeData[nodeName].usedMemory);
