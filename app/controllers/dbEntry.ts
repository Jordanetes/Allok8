...
const db2 = require("../models/dbmodel");

const dbEntry = {
  addAPI(req: Request, res: Response, next: NextFunction) {
  ...
  addNode(req: Request, res: Response, next: NextFunction){
    ...
    nodes.forEach((node) => {
      ...
      `,
        ...
        (err, sqlres) => {
          ...
          RETURNING pod_id
          `,
              [node_id, podName, timestamp, status],
              (err, sqlres) => {
                if (err) return next(err);
