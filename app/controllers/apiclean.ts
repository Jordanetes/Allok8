...
const cleaner = {
  ...
  nodeMetrics(req: Request, res: Response, next: NextFunction) {
    const { nodeInfo } = res.locals;
  ...
  podMetrics(pod: any, usedCPU: any, usedMemory: any) {
    ...
    for (let i = 0; i < spec.containers.length; i++) {
      ...
      formattedPod[podName].containers[
        containerName
      ].cpuRequested = cpuRequested;
    }
    return formattedPod;
  },
  podClean(req: Request, res: Response, next: NextFunction) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (let j = 0; j < pods.length; j++) {
        const podData = this.podMetrics(
          pods[j],
          nodeData[nodeName].usedCPU,
          nodeData[nodeName].usedMemory
        );
